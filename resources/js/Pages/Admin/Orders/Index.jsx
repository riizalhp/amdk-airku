...
// --- Komponen Form Pembuatan Pesanan ---
    ...
    return (
        <form onSubmit={submit} className="space-y-6">
            <div className="flex justify-between items-center pb-3 border-b">
                <h3 className="text-2xl font-bold text-gray-900">Buat Pesanan Baru</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select id="store_id" value={data.store_id} onChange={e => setData('store_id', e.target.value)} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                ...
                <div>
                    ...
                    <input type="date" id="order_date" value={data.order_date} onChange={e => setData('order_date', e.target.value)} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required />
            ...
            <div>
                ...
                <div className="flex gap-2 mt-1">
                    <select value={selectedProduct} onChange={e => setSelectedProduct(e.target.value)} className="flex-grow rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
            ...
            <div className="border rounded-lg overflow-hidden max-h-64 overflow-y-auto">
                <table className="min-w-full">
                    <thead className="bg-gray-50 sticky top-0">
                        <tr>
                            <th className="p-3 text-left text-xs font-bold text-gray-500 uppercase">Produk</th>
                            <th className="p-3 text-left text-xs font-bold text-gray-500 uppercase">Kuantitas</th>
                            <th className="p-3 text-left text-xs font-bold text-gray-500 uppercase">Subtotal</th>
                            <th className="p-3 w-16"></th>
                        </tr>
                    </thead>
                    <tbody>
                        {data.items.length === 0 && <tr><td colSpan="4" className="p-4 text-center text-gray-500">Belum ada produk ditambahkan.</td></tr>}
                        {data.items.map(item => (
                            <tr key={item.product_id} className="border-t">
                                <td className="p-3">{item.name}</td>
                                <td className="p-3"><input type="number" value={item.quantity} onChange={e => updateQuantity(item.product_id, e.target.value)} className="w-20 rounded-md border-gray-300" /></td>
                                <td className="p-3 font-medium">{formatCurrency(item.price * item.quantity)}</td>
                                <td className="p-3 text-center"><button type="button" onClick={() => removeItem(item.product_id)} className="text-red-500 hover:text-red-700">Hapus</button></td>
                            </tr>
                        ))}
                    </tbody>
                </table>
            ...
            <div className="flex justify-end gap-4 pt-4 border-t">
                ...
                <button type="submit" className="bg-blue-600 text-white font-bold py-2 px-4 rounded-lg shadow-md hover:bg-blue-700" disabled={processing || data.items.length === 0}>Simpan Pesanan</button>
            </div>
        </form>
    );
}
...
export default function Index({ auth, orders, summary, stores, products, flash }) {
    const [isModalOpen, setIsModalOpen] = useState(false);

    const header = (
        <div>
            <h2 className="font-bold text-2xl text-gray-800 leading-tight">Manajemen Pesanan</h2>
            <p className="text-sm text-gray-500">Kelola daftar pesanan dari semua toko</p>
        </div>
    );
    ...
    const getStatusBadge = (status) => {
        ...
        return <span className={`px-2.5 py-1 text-xs font-semibold rounded-full ${statuses[status] || 'bg-gray-100 text-gray-700'}`}>{statusText}</span>;
    };
        ...
        <AuthenticatedLayout user={auth.user} header={header}>
            ...
            <div className="p-4 md:p-8">
                ...
                <div className="flex flex-wrap justify-between items-center gap-4 mb-6">
                    <h3 className="text-xl font-bold text-gray-800">Riwayat Pesanan</h3>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {orders.map(order => (
                                <tr key={order.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4 text-sm text-gray-500">{order.user?.name || 'N/A'}</td>
                                    <td className="px-6 py-4 text-sm font-medium text-gray-800">{formatCurrency(order.total_amount)}</td>
                                    <td className="px-6 py-4 text-sm text-gray-500">{formatDate(order.order_date)}</td>
                                    <td className="px-6 py-4 text-sm text-gray-500">{getStatusBadge(order.status)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
